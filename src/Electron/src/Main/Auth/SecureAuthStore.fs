...
let private tryDecodeAuthMetadata (localSwateAccountId: string) (raw: string) : AuthMetadata option =
    try
        if not (isSafeLocalSwateAccountId localSwateAccountId) then
            ...
            match Thoth.Json.JavaScript.Decode.fromString (authMetadataDecoder localSwateAccountId) raw with
            | Ok metadata when metadata.Id > 0 -> Some metadata
            | _ -> None
    with _ ->
        None
...
let private tryDecodeActiveLocalSwateAccountId (raw: string) : string option =
    try
        Thoth.Json.JavaScript.Decode.fromString activeLocalSwateAccountIdDecoder raw
        |> function
            | Ok id -> id
            | Error _ -> None
    with _ ->
