...
let tryResolveLayerDestinations
    ...
    : Result<Map<ProvenanceSourceId, ProcessCoreProcessGroupLocation>, ProcessCoreCanonicalWritebackError list> =
    ...
    for layerId in session.LayerOrder do
        ...
        | None -> errors.Add(ProcessCoreCanonicalWritebackError.LayerNotFound layerId)
        | Some layer ->
            ...
            | None when not (isEditorCreatedLayer layer) ->
                errors.Add(ProcessCoreCanonicalWritebackError.SourceLocationNotFound layer.Source.Id)
            | None ->
                if String.IsNullOrWhiteSpace layer.Label then
                    errors.Add(ProcessCoreCanonicalWritebackError.BlankLayerName layer.Id)
                elif datasetPaths.Length <> 1 then
                    errors.Add(ProcessCoreCanonicalWritebackError.AmbiguousSourceLocation layer.Source.Id)
                else
                    let destination = {
