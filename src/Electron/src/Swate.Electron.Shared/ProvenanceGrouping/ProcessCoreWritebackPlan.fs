...
let private addError (errors: ResizeArray<ProcessCoreWritebackError>) value = errors.Add value

let private distinctErrors (errors: ResizeArray<ProcessCoreWritebackError>) = errors |> Seq.distinct |> Seq.toList

/// Editor-created layers have no ProcessCore source location at load time. Their
/// source ID is deliberately generated by the canonical layer command, so the
/// adapter can distinguish a new destination from a missing loaded source without
/// manufacturing a loaded location in the component model.
let isEditorCreatedLayer (layer: ProvenanceLayer) =
    layer.Source.Id = $"{layer.Id}:{layer.Label}"

/// Resolves every canonical layer to the destination that a writeback plan may
/// use. New layers are written to the one dataset context represented by the
/// loaded index, with their label becoming the new process-group name.
let tryResolveLayerDestinations
    (index: ProcessCoreWritebackIndex)
    (session: ProvenanceSession)
    : Result<Map<ProvenanceSourceId, ProcessCoreProcessGroupLocation>, ProcessCoreWritebackError list> =
    ...
    for layerId in session.LayerOrder do
        ...
        | None -> errors.Add(ProcessCoreWritebackError.LayerNotFound layerId)
        | Some layer ->
            match index.SourceLocations |> Map.tryFind layer.Source.Id with
            | Some location -> destinations <- destinations |> Map.add layer.Source.Id location
            | None when not (isEditorCreatedLayer layer) ->
                errors.Add(ProcessCoreWritebackError.SourceLocationNotFound layer.Source.Id)
            | None ->
                if String.IsNullOrWhiteSpace layer.Label then
                    errors.Add(ProcessCoreWritebackError.BlankLayerName layer.Id)
                elif datasetPaths.Length <> 1 then
                    errors.Add(ProcessCoreWritebackError.AmbiguousSourceLocation layer.Source.Id)
                else
                    let destination = {
                        DatasetPath = datasetPaths.Head
                    ...
                    if nameAlreadyExists || newLayerNames.Contains layer.Label then
                        errors.Add(ProcessCoreWritebackError.DuplicateLayerName layer.Label)
                    else
                        newLayerNames <- newLayerNames |> Set.add layer.Label

                    destinations <- destinations |> Map.add layer.Source.Id destination
...
let private tryStableRecipeResourceKey (errors: ResizeArray<ProcessCoreWritebackError>) context key =
...
let private tryStableRecipeKeyFromResource (errors: ResizeArray<ProcessCoreWritebackError>) context (recipe: Recipe) =
    try
...
let private isComponentKind =
    function
    | AssignmentPropertyKind.AdapterSpecific kind -> kind.Id = ProcessCoreKinds.componentKind.Id
    | AssignmentPropertyKind.Generic -> false

let private isRecipeKind =
    function
    | AssignmentPropertyKind.AdapterSpecific kind -> kind.Id = ProcessCoreKinds.processCoreRecipeKind.Id
    | AssignmentPropertyKind.Generic -> false

let private isRecipeReferenceValue =
    function
    | ProvenanceValue.Reference reference -> reference.Scheme = ProcessCoreKinds.processCoreRecipeScheme
    | _ -> false
...
let private assignmentsById (errors: ResizeArray<ProcessCoreWritebackError>) (session: ProvenanceSession) =
    ...
    for KeyValue(storedNodeId, node) in session.Nodes do
        if storedNodeId <> node.Id then
            addError
                errors
                (ProcessCoreWritebackError.InconsistentCanonicalState
    ...
    let rec resolve visited currentId =
        if visited |> Set.contains currentId then
            ...
            | first :: second :: _ ->
                addError
                    errors
                    (ProcessCoreWritebackError.ConflictingAnnotationIdentity(currentId, first.Payload, second.Payload))
...
let private validateCanonicalState (errors: ResizeArray<ProcessCoreWritebackError>) (session: ProvenanceSession) =
    ...
    |> List.iter (fun (key, nodes) ->
        ...
        if nodeIds.Length > 1 then
            addError errors (ProcessCoreWritebackError.ConflictingNodeIdentity($"{key.KindId}:{key.Name}", nodeIds))
    )
    ...
    for KeyValue(processId, structuralProcess) in session.Processes do
        if not (session.Layers.ContainsKey structuralProcess.OriginLayerId) then
            addError errors (ProcessCoreWritebackError.LayerNotFound structuralProcess.OriginLayerId)

        for KeyValue(storedLinkId, link) in structuralProcess.Links do
            if storedLinkId <> link.Id then
                addError errors (ProcessCoreWritebackError.InvalidProcessLink storedLinkId)
            ...
            for nodeId in nodeIds do
                if not (session.Nodes.ContainsKey nodeId) then
                    addError errors (ProcessCoreWritebackError.NodeNotFound nodeId)
        ...
        for KeyValue(_, assignment) in structuralProcess.Assignments do
            ...
            for linkId in assignment.CoveredLinkIds do
                if not (structuralProcess.Links.ContainsKey linkId) then
                    addError errors (ProcessCoreWritebackError.LinkNotFound linkId)

    for KeyValue(_, node) in session.Nodes do
        for KeyValue(_, assignment) in node.Assignments do
            if not (session.Values.ContainsKey assignment.ValueId) then
                addError errors (ProcessCoreWritebackError.ValueNotFound assignment.ValueId)
    ...
    for KeyValue(valueId, definition) in session.Values do
        ...
        if not (session.Properties.ContainsKey definition.PropertyId) then
            addError errors (ProcessCoreWritebackError.ValueNotFound definition.Id)
...
let private validateResourceJournal
    ...
    for mutation in session.MutationJournal do
        ...
        | ProvenanceMutation.ProcessAssignmentValueChanged(_, before, after, _) when
            ...
            ->
            addError errors (ProcessCoreWritebackError.ReadOnlyRecipeComponentMutation(Some after.Id))
        | ProvenanceMutation.ProcessAssignmentAdded(ownerId, assignment, context) when
            ...
            ->
            addError errors (ProcessCoreWritebackError.ReadOnlyRecipeComponentMutation(Some assignment.Id))
        | ProvenanceMutation.ProcessAssignmentRemoved(tombstone, context) when
            ...
            ->
            addError errors (ProcessCoreWritebackError.ReadOnlyRecipeComponentMutation(Some tombstone.Assignment.Id))
        | ProvenanceMutation.ProcessAssignmentCoverageChanged(ownerId, before, after, context) when
            componentAssignment before || componentAssignment after
            ->
        ...
        | ProvenanceMutation.ProcessAssignmentSplit(_, original, retained, split, _) when
            ...
            ->
            addError errors (ProcessCoreWritebackError.ReadOnlyRecipeComponentMutation(Some split.Id))
...
let private sourceNodeId (index: ProcessCoreWritebackIndex) (sourceLocation: ProcessCoreCanonicalNodeSourceLocation) =
    index.NodeLocations
...
let private indexedProcessLinkIds (index: ProcessCoreWritebackIndex) processId =
...
let private indexedLink (index: ProcessCoreWritebackIndex) processId linkId =
        ...
        | Some(resourceId, resource, location) ->
            ...
            let exactContainer =
                ...
                |> List.exists (fun (ownerId, recipe) ->
                    ...
                    && recipe.ReferenceSlotId = Some ProcessCoreKinds.processCoreExecutesRecipeSlot
            ...
            && tombstone.Assignment.Id = assignmentId
            && tombstone.Assignment.PropertyKind = AssignmentPropertyKind.AdapterSpecific ProcessCoreKinds.componentKind
            && tombstone.Assignment.CoveredLinkIds = expectedLinkIds
            && tombstone.Assignment.ReferenceSlotId.IsNone
...
let private additionalTypeForNode mappings propertyKind =
    ...
    | AssignmentPropertyKind.AdapterSpecific kind when kind.Id = ProcessCoreKinds.characteristic.Id ->
        Ok(Some "CharacteristicValue")
    | AssignmentPropertyKind.AdapterSpecific kind when kind.Id = ProcessCoreKinds.factor.Id -> Ok(Some "FactorValue")
    | AssignmentPropertyKind.AdapterSpecific kind when kind.Id = ProcessCoreKinds.parameter.Id ->
        Ok(Some "ParameterValue")
    | AssignmentPropertyKind.AdapterSpecific kind when kind.Id = ProcessCoreKinds.componentKind.Id ->
        Ok(Some "Component")
    | AssignmentPropertyKind.AdapterSpecific kind when kind.Id = ProcessCoreKinds.additionalProperty.Id -> Ok None
    | AssignmentPropertyKind.AdapterSpecific kind -> Error(ProcessCoreWritebackError.UnsupportedPropertyKind kind.Id)
...
let private plannedAnnotation
    ...
    | None ->
        addError errors (ProcessCoreWritebackError.ValueNotFound valueId)
        None
    | Some definition ->
        ...
        | Some property ->
            ...
            | ProvenanceValue.Reference _ ->
                addError errors (ProcessCoreWritebackError.UnsupportedPropertyKind "reference-as-annotation")
                    ...
                    | Some annotation ->
                        let targetDestination =
                            ...
                            | Some source ->
                                ...
                                | None ->
                                    addError errors (ProcessCoreWritebackError.SourceLocationNotFound source.Id)
...
let private resolveRecipeResource
    ...
    | [] ->
        ...
        addError errors (ProcessCoreWritebackError.RecipeResourceNotFound(reference.Scheme, key))

        None
    | _ :: _ :: _ ->
        addError
            errors
            (ProcessCoreWritebackError.AmbiguousRecipeResourceKey(reference.Scheme, candidates.Head.ResourceKey))
    ...
    | [ resource ] ->
        if recipePayloadFingerprint resource.Resource <> resource.LoadFingerprint then
            ...
            addError errors (ProcessCoreWritebackError.StaleRecipeResource(resource.Scheme, stableKey))
...
let private validateRecipeIndex (errors: ResizeArray<ProcessCoreWritebackError>) (index: ProcessCoreWritebackIndex) =
    for KeyValue((storedScheme, storedKey), resource) in index.RecipeResources do
        ...
        if recipeResourceIsMalformed resource then
            addError errors (ProcessCoreWritebackError.StaleRecipeResource(resource.Scheme, keyForError))
        else
    ...
    |> List.iter (fun ((scheme, _), resources) ->
        if resources.Length > 1 then
            addError errors (ProcessCoreWritebackError.AmbiguousRecipeResourceKey(scheme, resources.Head.ResourceKey))
    )
...
let private validateRecipeComponents
    ...
    | None when not bound.IsEmpty ->
        addError
            errors
            (ProcessCoreWritebackError.ReadOnlyRecipeComponentMutation(bound |> List.tryHead |> Option.map _.Id))
    | None -> ()
    | Some recipeAssignment ->
        ...
        for KeyValue(key, componentLocation) in expected do
            ...
            | None -> addError errors (ProcessCoreWritebackError.ReadOnlyRecipeComponentMutation None)
...
let private destinationForProcess
    ...
    | Some layer ->
        ...
        | None ->
            addError errors (ProcessCoreWritebackError.SourceLocationNotFound layer.Source.Id)
...
let private chooseReuseLink (index: ProcessCoreWritebackIndex) (structuralProcess: StructuralProcess) sourceProcess =
    let original =
        index.LinkLocations
        |> Map.toList
...
let private validateRecipeAssociationJournal
    ...
    let exactLoadedRecipeIdentity
        ...
        | Some processLocation ->
            ...
            && assignment.Lineage = AssignmentLineage.Loaded
            && isRecipeKind assignment.PropertyKind
            && assignment.ReferenceSlotId = Some ProcessCoreKinds.processCoreExecutesRecipeSlot
    ...
    for registryId, collisionGroup in annotations |> List.groupBy _.RegistryId do
        ...
        if divergent.Length > 1 then
            ...
            if uncontrolledFingerprints.Count > 1 then
                ...
                addError
                    errors
                    (ProcessCoreWritebackError.ConflictingAnnotationIdentity(
                        registryId,
        ...
        | Some destination ->
            ...
            let recipeAssignments =
                ...
                |> List.choose (fun (_, assignment) ->
                    ...
                    | Some({
                           ...
                           } as definition) when reference.Scheme = ProcessCoreKinds.processCoreRecipeScheme ->
                        if
                    ...
                    | Some {
                        ...
                        addError
                            errors
                            (ProcessCoreWritebackError.UnsupportedPropertyKind $"reference:{reference.Scheme}")
            ...
            for KeyValue(linkId, _) in structuralProcess.Links do
                ...
                if recipesForLink.Length > 1 then
                    addError errors (ProcessCoreWritebackError.InvalidProcessLink linkId)
            ...
            tryCreatePlan
                {
                    index with
                        ...
                        // editor-created destinations are never presented as
                        // loaded ProcessCore source locations.
                        SourceLocations = destinations
                }
                session
        )
